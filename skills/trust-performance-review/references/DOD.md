...
Declaring done with incomplete work is **worse** than failing loudly. The dev can investigate a halt â€” but cannot recover from silent incompleteness.

---

## Self-attestation block (required at end of execution)

At the end of your execution, append the following block to your output (or write it as `<run-dir>/agents/performance.dod.json`):
...
### `grounding.loaded`
`true` if you successfully loaded all docs listed in `trust.config.yaml > grounding.required`. `false` otherwise â†’ HALT.

### `grounding.docs_read[]`
For each loaded doc, record:
- `path` â€” relative path within the source
- `sha256` â€” hex hash of the file content (use `hashlib.sha256` in Python)
- `sections_indexed` â€” number of headings parsed
...
```
If you skipped any file in your domain for ANY reason, this drops below 100 â†’ HALT.
...
Boolean. `true` only if every `(rule Ã— file)` pair has a verdict.
...
The orchestrator reads `performance.dod.json` and:

- If `self_check_passed: false` â†’ triggers HALT, preserves artifacts, creates `.trust-halt` marker
- If `self_check_passed: true` â†’ proceeds to next phase

You are **not** the one who triggers HALT â€” the orchestrator is. Your job is to honestly attest.
