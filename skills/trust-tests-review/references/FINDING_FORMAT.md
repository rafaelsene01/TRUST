...
### `agent`
Always `"tests"`. Never another agent's name.

### `rule_id`
Must match a `rule_id` declared in `checklists/security.checklist.md`. Format: "TEST-XXX" where XXX is a 3-digit number. Never invent rule IDs.
