...
interface Finding {
  // Identification
  agent: "data";
  rule_id: string;              // e.g. "DAT-001"
  rule_source: string;          // e.g. "in-setup:04-data-model.md#<section>"
...
### `agent`
Always `"data"`. Never another agent's name.

### `rule_id`
Must match a `rule_id` declared in `checklists/security.checklist.md`. Format: "DAT-XXX" where XXX is a 3-digit number. Never invent rule IDs.
...
Example: `in-setup:04-data-model.md#<section>`
...
âœ… Good: `"Per 04-data-model.md#<section>, this project requires parameterized queries in 100% of DB access (no exceptions). This line allows trivial SQLi against the user lookup endpoint."`
...
```json
{
  "agent": "data",
  "rule_id": "DAT-001",
  "rule_source": "in-setup:04-data-model.md#<section>",
  ...
  "why_it_matters": "Per 04-data-model.md#<section>, this project requires parameterized queries in 100% of DB access. This line allows trivial SQLi against the user lookup endpoint.",
