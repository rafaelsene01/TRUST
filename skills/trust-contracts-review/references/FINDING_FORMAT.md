...
interface Finding {
  // Identification
  agent: "contracts";
  rule_id: string;              // e.g. "CON-001"
  rule_source: string;          // e.g. "in-setup:03-api-contracts.md#<section>"
...
### `agent`
Always `"contracts"`. Never another agent's name.

### `rule_id`
Must match a `rule_id` declared in `checklists/security.checklist.md`. Format: "CON-XXX" where XXX is a 3-digit number. Never invent rule IDs.
...
Example: `in-setup:03-api-contracts.md#<section>`
...
âœ… Good: `"Per 03-api-contracts.md#<section>, this project requires parameterized queries in 100% of DB access (no exceptions). This line allows trivial SQLi against the user lookup endpoint."`
...
```json
{
  "agent": "contracts",
  "rule_id": "CON-001",
  "rule_source": "in-setup:03-api-contracts.md#<section>",
  ...
  "why_it_matters": "Per 03-api-contracts.md#<section>, this project requires parameterized queries in 100% of DB access. This line allows trivial SQLi against the user lookup endpoint.",
