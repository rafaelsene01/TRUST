# FINDING_FORMAT — trust-security-review
...
interface Finding {
  // Identification
  agent: "security";
  rule_id: string;              // e.g. "SEC-007"
  rule_source: string;          // e.g. "in-setup:06-security-policy.md#input-validation"
...
### `agent`
Always `"security"`. Never another agent's name.

### `rule_id`
Must match a `rule_id` declared in `checklists/security.checklist.md`. Format: `SEC-XXX` where XXX is a 3-digit number. Never invent rule IDs.
...
Example: `in-setup:06-security-policy.md#input-validation`
...
- `critical` — exploitable vulnerability with immediate impact (SQLi, secret leak, auth bypass)
- `high` — serious risk but mitigated by other layers (weak crypto, missing rate limit)
- `medium` — defensive measure missing (verbose error messages, missing logging)
- `low` — style or hardening (could be better, not a real risk)
...
| < 0.80 | **DO NOT EMIT** — would be silenced anyway |
...
✅ Good: `"User input concatenated directly into SQL query — SQL injection vulnerability."`
❌ Bad: `"There might be a security issue with how the query is built."`
...
✅ Good: `"Per 06-security-policy.md#input-validation, this project requires parameterized queries in 100% of DB access (no exceptions). This line allows trivial SQLi against the user lookup endpoint."`

❌ Bad: `"SQL injection is a common vulnerability that can lead to data breaches."`

### `suggestion`
Proposed fix. Code example preferred. Don't propose changes that violate other project rules — check the conventions doc if uncertain.

### `false_positive_risk`
Honest assessment of when this finding could be wrong:

- `low` — the violation is unambiguous (`false_positive_reason: null`)
- `medium` — context could exonerate (e.g. "if `email` is pre-validated upstream, this is safe")
- `high` — heuristic match, plausible but not certain
...
✅ `"If the `email` field is validated by a middleware layer (not visible in this diff), the concatenation might be safe — though parameterization is still preferred per project policy."`
