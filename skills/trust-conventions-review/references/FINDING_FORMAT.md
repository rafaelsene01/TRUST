...
interface Finding {
  // Identification
  agent: "conventions";
  rule_id: string;              // e.g. "CONV-001"
  rule_source: string;          // e.g. "in-setup:05-conventions.md#<section>"
...
### `agent`
Always `"conventions"`. Never another agent's name.

### `rule_id`
Must match a `rule_id` declared in `checklists/security.checklist.md`. Format: "CONV-XXX" where XXX is a 3-digit number. Never invent rule IDs.
...
Example: `in-setup:05-conventions.md#<section>`
...
âœ… Good: `"Per 05-conventions.md#<section>, this project requires parameterized queries in 100% of DB access (no exceptions). This line allows trivial SQLi against the user lookup endpoint."`
...
```json
{
  "agent": "conventions",
  "rule_id": "CONV-001",
  "rule_source": "in-setup:05-conventions.md#<section>",
  ...
  "why_it_matters": "Per 05-conventions.md#<section>, this project requires parameterized queries in 100% of DB access. This line allows trivial SQLi against the user lookup endpoint.",
