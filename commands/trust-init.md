...
If no profile is specified, the command walks you through a wizard to choose.

---

## Execution (pilot profile)
...
```
<setup_path>/
├── README.md               (generated with team name + usage instructions)
├── trust.config.yaml       (generated from template)
├── .env.example            (template for other devs)
├── .env.local              (gitignored — absolute paths)
├── .gitignore
├── grounding/
│   ├── _drafts/            (empty — filled by /trust map)
│   ├── 01-architecture.md  (stub)
...
│       └── .gitkeep
├── checklists/
│   ├── security.checklist.md   (template with 10 starter rules)
│   ├── contracts.checklist.md
│   ├── data.checklist.md
│   ├── performance.checklist.md
│   ├── conventions.checklist.md
│   └── tests.checklist.md
├── skills/                 (empty — inherits from framework)
├── targets/
│   └── <repo-slug>.yaml    (one per detected repo)
└── runs/
    └── .gitkeep
```
